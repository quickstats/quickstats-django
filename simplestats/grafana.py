...
class GrafanaWidgetView(object):
    ...
    def query(self, request):
        ...
        for widget in self.get_queryset().filter(
            label__name="metric", label__value__in=targets
        ):
            response = {
                "target": widget.title,
            ...
            for dp in widget.sample_set.filter(
                timestamp__gte=start, timestamp__lte=end
            ).order_by("timestamp"):
                response["datapoints"].append([dp.value, self.__ts(dp.timestamp)])
                ...
                results.append(
                    {
                        ...
                        "tags": [
                            "{x.name}:{x.value}".format(x=x)
                            for x in annotation.widget.label_set.all()
                        ],
